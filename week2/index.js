...
if (playerA === '剪刀') {
  ...
  else if(playerB === '石頭'){console.log('playerA輸,playerB贏');} 
  else{console.log('playerA贏,playerB輸');}
} else if(playerA === '石頭'){
  if (playerB === '剪刀') {console.log('playerA贏,playerB輸');} 
  else if(playerB === '石頭'){console.log('平手');} 
  else{console.log('playerA輸,playerB贏');}
} else{
  if (playerB === '剪刀') {console.log('playerA輸,playerB贏');} 
  else if(playerB === '石頭'){console.log('playerA贏,playerB輸');} 
  else{console.log('平手');}
}
...
let targetPerformance = 50000

// 練習：第一位教練（可將下方程式碼註解移除，完成答題）
if (performanceData.coaches[0].performance < targetPerformance) {
    // 請填寫第一位教練業績增長程式碼，使用 +=
    performanceData.coaches[0].performance += (targetPerformance - performanceData.coaches[0].performance);
}
